...
const GitHubPulse = () => {
  const { publicRepos, totalStars, topLanguages, recentActivity, loading, error } = useGitHubStats('Sarthaksk26');
  const magneticRef = useMagnetic();

  const getEventIcon = (type: string) => {
    switch (type) {
      case 'PushEvent': return <GitCommit size={14} className="text-emerald-500" />;
      case 'PullRequestEvent': return <GitPullRequest size={14} className="text-blue-500" />;
      case 'CreateEvent': return <BookOpen size={14} className="text-purple-500" />;
      case 'WatchEvent': return <Star size={14} className="text-yellow-500" />;
      default: return <Github size={14} className="text-slate-500" />;
    }
  };
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 text-lg max-w-2xl">
              A live dashboard of my open-source activity, languages, and recent contributions pulled directly from GitHub.
            </p>
          </div>
            ...
            <div className="glass p-8 rounded-[2.5rem] border border-slate-200 dark:border-white/10 flex flex-col justify-center">
              <div className="grid grid-cols-2 gap-8 text-center">
                <div className="space-y-2">
                  ...
                  <div className="text-sm font-bold text-slate-500 uppercase tracking-widest">Public Repos</div>
                </div>
                <div className="space-y-2">
                  <div className="text-5xl font-bold text-yellow-500">{totalStars}</div>
                  <div className="text-sm font-bold text-slate-500 uppercase tracking-widest">Total Stars</div>
                </div>
              </div>
            </div>
