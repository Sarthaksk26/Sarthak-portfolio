    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="rounded-xl border border-slate-200 dark:border-white/10 bg-slate-900 overflow-hidden flex flex-col">
        ...
        <div className="p-4 bg-[#0a0a0a] font-mono text-sm leading-relaxed flex-1">
          <div className="text-white">
            console.log(<span className="text-emerald-400">'Start'</span>);
          </div>
          <br />
          <div className="text-blue-400">
            setTimeout(<span className="text-white">() =&gt;</span> {'{'}
          </div>
          <div className="pl-4 text-white">
            console.log(<span className="text-emerald-400">'Timeout'</span>);
          </div>
          <div className="text-blue-400">
          ...
          <div className="text-yellow-400">
            Promise.resolve().then(<span className="text-white">() =&gt;</span> {'{'}
          </div>
          <div className="pl-4 text-white">
            console.log(<span className="text-emerald-400">'Promise'</span>);
          </div>
          <div className="text-yellow-400">{'}'});</div>
          <br />
          <div className="text-white">
            console.log(<span className="text-emerald-400">'End'</span>);
          </div>
        </div>
      </div>
      ...
      <div className="lg:col-span-2 grid grid-cols-2 gap-4">
        ...
        <div className="border border-slate-200 dark:border-white/10 rounded-xl bg-slate-50 dark:bg-white/5 p-4 flex flex-col">
          <h4 className="text-sm font-bold text-slate-500 mb-4 text-center uppercase tracking-widest">
            Call Stack
          </h4>
          <div className="flex-1 flex flex-col-reverse gap-2 border-x-2 border-b-2 border-slate-300 dark:border-slate-700 p-2 min-h-[150px]">
            {callStack.map((t) => (
              <div
                key={t.id}
                className="bg-accent text-white p-2 rounded text-center text-sm font-mono animate-fade-slide-up shadow-md"
              >
                {t.name}
              </div>
            ))}
        ...
        <div className="border border-slate-200 dark:border-white/10 rounded-xl bg-slate-50 dark:bg-white/5 p-4 flex flex-col">
          ...
          <div className="flex-1 border-2 border-dashed border-slate-300 dark:border-slate-700 p-2 min-h-[150px] rounded-lg flex flex-col gap-2">
            {webApi.map((t) => (
              <div
                key={t.id}
                className="bg-blue-500 text-white p-2 rounded text-center text-sm font-mono animate-fade-in"
              >
                {t.name}
              </div>
            ))}
        ...
        <div className="border border-slate-200 dark:border-white/10 rounded-xl bg-slate-50 dark:bg-white/5 p-4 flex flex-col col-span-2">
          ...
          <div className="flex gap-2 border-2 border-slate-300 dark:border-slate-700 p-2 min-h-[60px] rounded-lg overflow-x-auto">
            {taskQueue.map((t) => (
              <div
                key={t.id}
                className="bg-purple-500 text-white p-2 rounded text-center text-sm font-mono animate-fade-in whitespace-nowrap"
              >
                {t.name}
              </div>
            ))}
